...
const likeProduct = async (req, res) => {
  try {
    ...
    if (existingLike) {
      ...
      // Decrement the like count in the Product model if it's positive
      ...
      // Ensure totalLikes is non-negative
      totalLikes = Math.max(0, product.numOfLikes);

      await Notification.deleteOne({ sender: userId, receiver: product.user, type: "likeProduct", postId: productId });

      res
        .status(StatusCodes.OK)
        .json({ message: "Product unliked successfully", numOfLikes: totalLikes });
    } else {
      ...
      await Notification.create({ sender: userId, receiver: product.user, type: "likeProduct", postId: productId });

      res
        .status(StatusCodes.OK)
        .json({ message: "Product liked successfully", numOfLikes: totalLikes });
    }
  } catch (error) {
...
const replyToPost = async (req, res) => {
  try {
    ...
    await Notification.create({ sender: userId, receiver: post.user, type: "replyToPost", postId });

    // Get the updated post with replies
    const updatedPost = await Product.findById(postId).populate('replies.userId', 'username'); // Assuming 'replies.userId' is the reference to the user who replied
...
const replyToReply = async (req, res) => {
  try {
    ...
    await Notification.create({ sender: userId, receiver: parentReply.userId, type: "replyToReply", postId });
    ...
    res.status(200).json({ replies: allReplies }); // Include all replies related to the postId
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
